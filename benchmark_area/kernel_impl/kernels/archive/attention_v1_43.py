"""attention_v1.43 — v1.42 with fused copy kernel (single launch instead of 2 memcpy).

Replaces the two separate ``cudaMemcpyAsync`` calls for staging ``q`` and
``packed_th`` with a single Triton copy kernel, saving ~0.8 µs of GPU
launch overhead per query.
"""
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_43_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v1_43 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v1_43 requires build_v2-style state")
    ...
    if th_per_subspace.dim() != 2 or th_per_subspace.shape[1] != h_q:
        raise RuntimeError(
            "attention_v1_43 expects packed fp16 thresholds with shape (2*S, H_q)"
        )
    rows = int(th_per_subspace.shape[0])
    s_hint = rows // 2
    if rows % 2 != 0 or s_hint not in _v31._ALLOWED_S:
        raise RuntimeError(
            f"attention_v1_43 expects packed fp16 thresholds with 2*S rows "
    ...
    cache = state.setdefault("_attn_v1_43_fixed", {})
    ...
    if cache.get("key") != cache_key or fixed is None:
        ...
        if groups > _GROUPS_MAX:
            raise RuntimeError(
                f"attention_v1_43 requires groups <= {_GROUPS_MAX}; got {groups}"
            )
    ...
    cfg.update((state.get("_attn_v1_43_buffer_cfg") or {}).get(bucket, {}))
    _capture_graph(
