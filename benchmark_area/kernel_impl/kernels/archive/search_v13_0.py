"""search_v13.0 — anchor-only full pass with bitmask gating.

Design:
  - Compute anchor-subspace cluster pass for all parents only.
  - Store anchor pass as one int32 bitmask per (kv head, parent).
  - Evaluate non-anchor upper bounds on demand inside the search kernel using
    each child's assigned clusters.

This removes the full materialization of cluster_pass for non-anchor
subspaces while also shrinking the anchor pass representation.
"""
...
def _anchor_pass_bitmask(
    q_sub: torch.Tensor,          # (H_q, d_sub)
    th_sub: torch.Tensor,         # (H_q,)
    centers_sub: torch.Tensor,    # (H_kv, K, d_sub)
    radii_sub: torch.Tensor,      # (H_kv, K)
    groups: int,
) -> torch.Tensor:
    ...
    if groups > 31:
        raise RuntimeError(f"search_v13 only supports up to 31 query groups, got {groups}")
...
def _get_or_make_block_pack(state: dict) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    cache = state.setdefault("_search_v13_0_block_pack", {})
    cache_key = (
        keys_reord.data_ptr(),
...
if HAS_TRITON:
    ...
    @triton.jit
    def _anchor_mask_ondemand_kernel(
        Q_ptr,              # (H_q, D)
        ...
        anchor_mask = tl.load(
            AnchorMask_ptr + kvh * K + parent_idx_safe,
            mask=col_valid,
            other=0,
        ).to(tl.int32)
        child_mask = anchor_mask
        ...
        child_mask = tl.where((inv == 0) & col_valid, child_mask, 0)
        ...
        survive2d = (
            out_mask
            & ((((child_mask[None, :] >> g_range[:, None]) & 1) != 0))
        )
        out = tl.where(survive2d, acc, neg_inf)
        tl.store(Out_ptr + out_offs, out, mask=out_mask)


def _get_layout_v13(state: dict, q_head_to_kv: torch.Tensor | None, q: torch.Tensor):
    ...
    cache = state.setdefault("_search_v13_0_cache", {})
    cache_key = (
        mode,
        groups,
        mapping_sig,
...
def search(
    ...
    if not HAS_TRITON:
        raise RuntimeError("search_v13 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("search_v13 requires build_v2-style state")

    layout = _get_layout_v13(state, q_head_to_kv, q)
    h_q = q.shape[0]
    ...
    anchor_mask = _anchor_pass_bitmask(
        q_sub=q[:, s0:e0].contiguous(),
        th_sub=th_packed[anchor_s].contiguous(),
        centers_sub=layout["centers"][anchor_s, :, :, : e0 - s0].contiguous(),
        radii_sub=layout["radii"][anchor_s].contiguous(),
    ...
    _anchor_mask_ondemand_kernel[grid](
        q.contiguous(),
        q_packed,
        q_norm,
        th_packed,
