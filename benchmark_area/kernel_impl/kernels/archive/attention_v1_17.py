"""attention_v1.17 — v1.16 with a leaner cluster-pass lane shape.

The only math change is in cluster_pass: use a GROUPS_POW floor of 4 instead
of 8 so grouped-query cases like H_q=24/H_kv=8 do less idle-lane work.
"""
...
def _parents_per_prog_for_bf(bf: int, groups: int) -> int:
    # Wider parent tiles help the high-group BF=4 case (for example Qwen's
    # H_q=28/H_kv=4) by reducing loop overhead in the sparse index kernel
    # without hurting the lower-group Llama-style layouts.
...
def _bucket_for(l_buf: int) -> int:
    for b in _BUCKETS:
        if l_buf <= b:
            return b
    raise ValueError(
        f"attention_v1_17: buffer length {l_buf} exceeds max bucket {_BUCKETS[-1]}"
    )
...
def _require_supported(layout: dict) -> None:
    ...
    if s not in _ALLOWED_S:
        raise RuntimeError(
            f"attention_v1_17 requires S in {_ALLOWED_S}; got S={s}"
        )
    if int(layout["groups"]) > _GROUPS_MAX:
        raise RuntimeError(
            f"attention_v1_17 requires groups <= {_GROUPS_MAX}; got {layout['groups']}"
        )


def _buffer_cfg(state: dict, bucket: int) -> dict[str, int]:
    override = state.get("_attn_v1_17_buffer_cfg") or {}
...
def _capture_no_buffer_graph(
    ...
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_17_use_cuda_graphs", True)):
...
def _get_fixed_runtime(
    ...
    cache = state.setdefault("_attn_v1_17_fixed", {})
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v1_17 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v1_17 requires build_v2-style state")

    h_q = q.shape[0]
    d = q.shape[1]
    q_c = q if q.is_contiguous() else q.contiguous()

    fixed_probe = state.get("_attn_v1_17_fixed", {}).get("fixed")
    ...
    if s_hint is None:
        ...
        if s_hint not in _ALLOWED_S:
            raise RuntimeError(
                f"attention_v1_17 requires S in {_ALLOWED_S}; inferred S={s_hint}"
            )
