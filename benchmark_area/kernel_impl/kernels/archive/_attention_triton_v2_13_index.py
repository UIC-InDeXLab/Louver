"""v2.13 fused index+buffer kernel — inline anchor gate + guarded AND filter.

Same as v2.11 but the multi-subspace AND filter (S-1 scattered load pairs)
only runs inside the anchor liveness guard.  Since the anchor gate typically
prunes 80-90% of chunks, the AND filter cost is amortized over very few
chunks, keeping the kernel close to v2.6 speed.
"""
...
if HAS_TRITON:
    ...
    @triton.jit
    def _fused_attn_index_buf_and_guarded_v2_13_kernel(
        Q_ptr,
        KeysBlocksT_ptr,
        ValuesBlocks_ptr,
        CentersAnchor_ptr,
        ...
        for p_chunk_start in range(p_start, p_end, PARENTS_PER_PROG):
            ...
            if tl.max(live_cols.to(tl.int32), axis=0) != 0:
                # AND with non-anchor subspaces (only for chunks surviving anchor)
...
def run_fused_attn_index_buf_and_guarded_v2_13(
    q: torch.Tensor,
    ...
    _fused_attn_index_buf_and_guarded_v2_13_kernel[grid](
        q,
        keys_blocks_t_f16,
        values_blocks_f16,
        centers_anchor,
