"""attention_v2.15 — optimized AND filter: tl.dot cluster_pass + child_survive.

Pipeline (4 launches, all captured in a single CUDA graph):
  1. cluster_pass (tl.dot):  uses tensor cores instead of element-wise mul+sum
  2. child_survive:  pre-computed (H_q, K*BF) int8 mask
  3. fused_index+buffer:  reads child_survive per chunk
...
def _make_shared_v2_15(layout, q, th_view, num_splits):
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v2_15_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v2_15 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v2_15 requires build_v2-style state")
    ...
    cache = state.setdefault("_attn_v2_15_fixed", {})
    ...
    if cache.get("key") != cache_key or fixed is None:
        ...
        shared = _make_shared_v2_15(layout, q_c, th_view, num_splits)
        fixed = {
