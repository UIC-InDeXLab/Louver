"""attention_v2.5 — v2.0 (exp2) with tuned buffer: cols=128, warps=4 for bucket 256.

Wider buffer tiles (128 cols) halve the loop iterations for 256-entry
buffers.  Fewer warps (4) free registers for the wider tile.
"""
...
_BUFFER_CFG: dict[int, dict[str, int]] = {
    64: {"cols": 64, "num_warps": 4, "num_stages": 4},
    128: {"cols": 128, "num_warps": 4, "num_stages": 3},
    256: {"cols": 128, "num_warps": 4, "num_stages": 3},
    512: {"cols": 128, "num_warps": 4, "num_stages": 3},
}
...
def _launch_with_buffer(shared, stage, layout, h_q, k, groups, groups_pow,
    ...
    run_attn_reduce_buffer_v2_0(
        ...
        l_buf_max=bucket, buf_cols_per_prog=buf_cols,
        scale_log2e=scale_log2e, out=shared["out"],
        groups_tile=4, num_warps=buf_warps, num_stages=buf_stages,
    )
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v2_5_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v2_5 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v2_5 requires build_v2-style state")
    ...
    cache = state.setdefault("_attn_v2_5_fixed", {})
