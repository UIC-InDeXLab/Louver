"""v1.18.1 sparse attention kernel with fp16 query inputs.

This keeps v1.14/v1.18's split-parallel sparse path, but expects the caller
to provide already-converted fp16 queries so the timed path does not cast
...
if HAS_TRITON:
    ...
    @triton.jit
    def _fused_attn_index_fp16q_ns_kernel(
        Q_ptr,
        KeysBlocksT_ptr,
        ValuesBlocks_ptr,
        AssignsBlocks_ptr,
...
def run_fused_attn_index_fp16q_ns(
    q: torch.Tensor,
    ...
    _fused_attn_index_fp16q_ns_kernel[grid](
