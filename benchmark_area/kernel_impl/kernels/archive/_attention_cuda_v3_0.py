"""JIT loader for the v3.0 native CUDA fused attention kernels."""
...
_SRC = Path(__file__).with_name("_cuda_attn_v3_0.cu")
...
def load_ext():
    ...
    try:
        ...
        return _EXT
    except Exception as exc:  # pragma: no cover - build failure depends on env
        _EXT_ERROR = exc
        raise
...
def run_fused_attn_index_cuda_v3_0(
    q: torch.Tensor,
...
def run_attn_reduce_buffer_cuda_v3_0(
    q: torch.Tensor,
    m_idx: torch.Tensor,
    l_idx: torch.Tensor,
    o_idx: torch.Tensor,
    buf_keys: torch.Tensor,
    buf_values: torch.Tensor,
    buf_invalid: torch.Tensor,
    num_splits: int,
    groups: int,
    scale: float,
    out: torch.Tensor,
    ...
    mod = load_ext()
    mod.sparse_attn_v3_0_reduce_buffer(
        q,
        m_idx,
        l_idx,
        o_idx,
