"""attention_v2.24 — Fused cluster_pass + child_survive (3 graph launches).

Pipeline (3 launches in CUDA graph):
  1. fused_survive:  computes cluster_pass inline + child_survive in one kernel
  2. fused_index+buffer:  reads child_survive per chunk
  3. reduce (exp2)

Saves one kernel launch vs v2.15 (4 launches).
"""
...
from .attention_v1_40 import _make_shared_workspace
from ._attention_copy_triton import fused_copy_q_th
from ._attention_triton_v2_24_fused_survive import run_fused_survive
...
def _launch_with_buffer(shared, stage, layout, h_q, k, groups, groups_pow,
    ...
    run_fused_survive(
        ...
        radii=layout["radii"], assigns_blocks=layout["assigns_blocks"],
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v2_24_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("v2.24 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("v2.24 requires build_v2-style state")
    ...
    cache = state.setdefault("_attn_v2_24_fixed", {})
    ...
    bucket = _bucket_for(l_buf) if not _empty_buffer(buffer_keys, buffer_values) else 256
