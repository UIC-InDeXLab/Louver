"""attention_v1.36 — tensor-core anchor pass + parent-gated anchor index."""
...
from ._attention_triton_v1_36_cluster import triton_anchor_cluster_pass_rawq_fp16_tc
from ._attention_triton_v1_36_index import run_fused_attn_index_anchor_fp16q_parentgate
...
def _choose_num_splits(
    ...
    override = state.get("_attn_v1_36_num_splits")
    if override is not None:
        return int(override)
    max_splits = int(state.get("_attn_v1_36_max_splits", max(requested, 64)))
...
def _parents_per_prog_for_v136(groups: int) -> int:
    if groups >= 6:
        return 32
    return _PARENTS_PER_CHUNK
...
def _anchor_layout(layout: dict) -> dict:
    ...
    cached = layout.get("_attn_v1_36_anchor")
    ...
    layout["_attn_v1_36_anchor"] = cached
    return cached
...
def _launch_no_buffer(
    ...
    triton_anchor_cluster_pass_rawq_fp16_tc(
        q=shared["static_q"],
    ...
    run_fused_attn_index_anchor_fp16q_parentgate(
        q=shared["static_q"],
...
def _capture_no_buffer_graph(
    ...
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_36_use_cuda_graphs", True)):
...
def attend(
    ...
    if not HAS_TRITON:
        raise RuntimeError("attention_v1_36 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v1_36 requires build_v2-style state")
    ...
    if th_per_subspace.dim() != 2 or th_per_subspace.shape[1] != h_q:
        raise RuntimeError(
            "attention_v1_36 expects packed fp16 thresholds with shape (2*S, H_q)"
        )
    rows = int(th_per_subspace.shape[0])
    s_hint = rows // 2
    if rows % 2 != 0 or s_hint not in _v31._ALLOWED_S:
        raise RuntimeError(
            f"attention_v1_36 expects packed fp16 thresholds with 2*S rows "
    ...
    parents_per_prog = int(state.get("_attn_v1_36_parents_per_prog", _parents_per_prog_for_v136(int(layout["groups"]))))
    num_splits_eff = _choose_num_splits(state, q_c, layout, num_splits, parents_per_prog)
    ...
    if _empty_buffer(buffer_keys, buffer_values):
        stage = fixed.get("v1_36_no_buffer_stage")
        if stage is None:
            stage = {"graph": None, "capture_failed": False}
            fixed["v1_36_no_buffer_stage"] = stage
        _capture_no_buffer_graph(
            state, shared, stage, layout, h_q, k, groups, groups_pow,
            num_splits_eff, anchor_s, scale, parents_per_prog, anchor,
        )
        try:
            ...
            state["_attn_v1_36_disabled"] = True
            return _v31.attend(
                q=q,
    ...
    bucket_stages = fixed.setdefault("v1_36_buckets", {})
    ...
    cfg.update((state.get("_attn_v1_36_buffer_cfg") or {}).get(bucket, {}))
