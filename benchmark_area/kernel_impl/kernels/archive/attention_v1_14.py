"""attention_v1.14 — v1.8 + num_stages=2 pipelining on main attn kernel."""
...
def _maybe_capture_graph(
    ...
    if work["graph"] is not None or work["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_14_use_cuda_graphs", True)):
...
def _get_fixed_empty_runtime(
    ...
    cache = state.setdefault("_attn_v1_14_fixed", {})
...
def attend(
    ...
    if not _empty_buffer(buffer_keys, buffer_values):
        raise RuntimeError("attention_v1_14 is a fixed-shape empty-buffer variant")
