"""v2.14 fused index+buffer kernel — anchor gate + guarded child_survive load.

Hybrid of v2.6 and v2.12:
 - Inline anchor gate (cheap dot product) prunes ~80-90% of parent chunks
 - Only for surviving chunks: load pre-computed child_survive mask to AND
   across all subspaces
 - Buffer scan phase (same as v2.6)

The child_survive load (1KB per chunk) only executes for the ~10-20% of
chunks that survive the anchor gate, keeping overhead minimal.
"""
...
if HAS_TRITON:
    ...
    @triton.jit
    def _fused_attn_index_buf_v2_14_kernel(
        Q_ptr,
        KeysBlocksT_ptr,
        ValuesBlocks_ptr,
        CentersAnchor_ptr,
        ...
        for p_chunk_start in range(p_start, p_end, PARENTS_PER_PROG):
            ...
            # Inline anchor gate (cheap, tensor-core dot product)
            centers = tl.load(
                CentersAnchor_ptr
            ...
            if tl.max(live_cols.to(tl.int32), axis=0) != 0:
                # AND with all subspaces via pre-computed child_survive
                child_global = parent_idx_safe * BF + child_rel
                cs_raw = tl.load(
                    ChildSurvive_ptr
                    + hq_vec[:, None] * TOTAL_CHILDREN
                    + child_global[None, :],
                    mask=g_valid[:, None] & col_valid[None, :],
                    other=0,
                )
                survive = survive & (cs_raw != 0)
...
def run_fused_attn_index_buf_v2_14(
    q: torch.Tensor,
    ...
    _fused_attn_index_buf_v2_14_kernel[grid](
        q,
        keys_blocks_t_f16,
        values_blocks_f16,
        centers_anchor,
