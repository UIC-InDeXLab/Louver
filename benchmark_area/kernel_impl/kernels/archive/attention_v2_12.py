"""attention_v2.12 — v2.6 speed + full multi-subspace AND filter.
...
Pipeline (4 launches, all captured in a single CUDA graph):
  ...
  3. fused_index+buffer: reads child_survive with one load per chunk
     instead of 2*(S-1) scattered gathers — same buffer fusion as v2.6
...
def _make_shared_v2_12(layout, q, th_view, num_splits):
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v2_12_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v2_12 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v2_12 requires build_v2-style state")
    ...
    cache = state.setdefault("_attn_v2_12_fixed", {})
    ...
    if cache.get("key") != cache_key or fixed is None:
        ...
        shared = _make_shared_v2_12(layout, q_c, th_view, num_splits)
        fixed = {
