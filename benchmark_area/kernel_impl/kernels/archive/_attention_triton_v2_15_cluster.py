"""v2.15 optimized cluster_pass kernel — uses tl.dot (tensor cores) instead
of element-wise multiply + sum.  Also pre-computes child_survive inline,
merging two kernel launches into one.

Grid: (H_Q, cdiv(TOTAL_CHILDREN, BLOCK_C))
Each thread block handles one query head and a block of children.
For each child in the block, it computes the full multi-subspace AND
survive mask, writing directly to the child_survive output.
"""
