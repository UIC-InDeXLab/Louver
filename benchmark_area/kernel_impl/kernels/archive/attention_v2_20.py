"""attention_v2.20 — v2.14 hybrid (anchor+guarded child_survive) + tl.dot cluster_pass.

Pipeline (4 launches, all in CUDA graph):
  1. cluster_pass (tl.dot)
  2. child_survive
  3. fused_index+buffer: anchor gate → guarded child_survive load
  4. reduce (exp2)
"""
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v2_20_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v2_20 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v2_20 requires build_v2-style state")
    ...
    cache = state.setdefault("_attn_v2_20_fixed", {})
