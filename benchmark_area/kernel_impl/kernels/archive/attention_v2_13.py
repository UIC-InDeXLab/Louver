"""attention_v2.13 — v2.11 with AND filter guarded behind anchor liveness check.

Pipeline (3 launches, all captured in a single CUDA graph):
  1. cluster_pass: per-subspace parent gating (reused from v1.31)
  2. fused_index+buffer+AND(guarded): inline anchor gate checks first,
     AND filter only runs for chunks surviving the anchor gate (~10-20%)
...
from ._attention_triton_v2_13_index import run_fused_attn_index_buf_and_guarded_v2_13
...
def _launch_with_buffer(shared, stage, layout, h_q, k, groups, groups_pow,
    ...
    run_fused_attn_index_buf_and_guarded_v2_13(
        q=shared["static_q"],
        keys_blocks_t_f16=layout["keys_blocks_t_f16"],
        values_blocks_f16=layout["values_blocks_f16"],
        centers_anchor=anchor["centers"], radii_anchor=anchor["radii"],
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v2_13_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v2_13 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v2_13 requires build_v2-style state")
    ...
    cache = state.setdefault("_attn_v2_13_fixed", {})
