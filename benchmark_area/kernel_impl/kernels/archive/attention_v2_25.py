"""attention_v2.25 — 3-launch AND filter pipeline.

Pipeline (3 launches in CUDA graph):
  1. cluster_pass (tl.dot)
  2. fused_index+AND_from_cluster_pass + buffer
  3. reduce (exp2)

Eliminates the child_survive kernel by reading cluster_pass directly
in the index kernel, guarded by the inline anchor gate.
"""
...
from ._attention_triton_v2_25_index import run_fused_attn_index_buf_v2_25
...
def _launch_with_buffer(shared, stage, layout, h_q, k, groups, groups_pow,
    ...
    run_fused_attn_index_buf_v2_25(
        q=shared["static_q"],
        ...
        width=int(anchor["centers"].shape[-1]),
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v2_25_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("v2.25 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("v2.25 requires build_v2-style state")
    ...
    cache = state.setdefault("_attn_v2_25_fixed", {})
