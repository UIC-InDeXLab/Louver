// attention v1.2 — AVX-512 inner kernels for D=128, width=16.
//
// v1.1 left scalar dot products on the table; on real captures the inner
// D=128 dot is the dominant time. AVX-512 collapses that 128-op scalar loop
// into 8 FMAs across 8 zmm registers (16 fp32 each).
//
// Hot paths specialized for D=128 / width=16 (Llama, Qwen, etc.); generic
// scalar fallback kept for any other shape.
//
// Inherits the v1.1 structure (single-pass online softmax, parent-tile
// parallelism). Only the inner per-row math is changed.
...
// 128-wide dot via 4 zmm accumulators (better ILP than 1 acc).
...
// Generic dot: handles tail. Used for non-128 D or non-16 widths.
...
// AXPY for D=128: dst[i] += w * src[i]. Used for value accumulation.
...
// Scale + axpy: dst[i] = a*dst[i] + b*src[i]. Used by online softmax rescale.
...
torch::Tensor attend_v1_2(
    ...
    for (int64_t task = 0; task < total_tiles; ++task) {
        ...
        for (int64_t s = 0; s < s_count; ++s) {
            ...
            const float r = dot_dispatch(qp + qh * d + st, qp + qh * d + st, static_cast<int>(w));
            q_norms[s] = std::sqrt(std::max(r, 0.0f));
        }
        ...
        for (int64_t j = row_lo; j < row_hi; ++j) {
            ...
            for (int64_t s = 0; s < s_count; ++s) {
                ...
                float bound = dot_dispatch(qp + qh * d + st, cp, static_cast<int>(w));
            ...
            float score = dot_dispatch(qp + qh * d, key, static_cast<int>(d)) * static_cast<float>(scale);
    ...
    for (int64_t qh = 0; qh < h_q; ++qh) {
        ...
        for (int64_t j = 0; j < n_buf; ++j) {
            ...
            const float score = dot_dispatch(qp + qh * d, key, static_cast<int>(d)) * static_cast<float>(scale);
