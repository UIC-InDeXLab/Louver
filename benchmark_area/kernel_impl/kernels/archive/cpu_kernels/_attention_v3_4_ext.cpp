// attention v3.4 — Precomputed per-child AND bitmask + GQA-tiled scoring:
//   1. Fused q_norms + parent pass (single OMP pass)
//   2. Precompute per-child per-qh AND bitmask in dedicated OMP pass
//      (checks all subspaces once, builds compact bitmask)
//   3. GQA-tiled scoring: just check one bitmask bit per child per group member
//   4. Parent-block skipping via precomputed bitmask OR
#include "_attention_v2_exact_common.h"

namespace {
constexpr int64_t V34_PARENTS_PER_TILE = 64;
}

torch::Tensor attend_v3_4(
    ...
    TORCH_CHECK(max_groups <= V2_MAX_GROUPS, "v3.4 max GQA group size ", V2_MAX_GROUPS);
    ...
    // --- Phase 1: Fused q_norms + parent pass tables ---
    ...
    // --- Phase 2: Precompute per-child AND bitmask ---
    // child_pass[kvh * n_pad + j] = bitmask of which qh (within kvh's GQA group) pass all subspaces
    // Also compute parent_any[kvh * k_total + parent] = OR of child_pass bits for any child in block
    std::unique_ptr<uint32_t[]> child_pass(
        new uint32_t[static_cast<size_t>(p.h_kv * p.n_pad)]);
    std::fill(child_pass.get(), child_pass.get() + p.h_kv * p.n_pad, 0u);

    // Parallelize over (kvh, parent_block) for good granularity
    const int64_t total_parents = p.h_kv * p.k_scan;
    #pragma omp parallel for schedule(dynamic)
    for (int64_t task = 0; task < total_parents; ++task) {
        const int64_t kvh = task / p.k_scan;
        const int64_t parent = task % p.k_scan;
        ...
        for (int64_t child = 0; child < p.bf; ++child) {
            ...
            child_pass[kvh * p.n_pad + j] = mask;
        }
    }

    // Precompute parent_any: bitmask OR of all children in a parent block
    std::unique_ptr<uint32_t[]> parent_any(
        new uint32_t[static_cast<size_t>(p.h_kv * p.k_total)]);
    std::fill(parent_any.get(), parent_any.get() + p.h_kv * p.k_total, 0u);
    ...
    for (int64_t task = 0; task < total_parents; ++task) {
        ...
        for (int64_t child = 0; child < p.bf; ++child) {
            const int64_t j = parent * p.bf + child;
            if (j >= p.n_scan) break;
            any |= child_pass[kvh * p.n_pad + j];
        }
        parent_any[kvh * p.k_total + parent] = any;
    }

    // --- Phase 3: GQA-tiled scoring — just check bitmask ---
    const int64_t n_tiles = (p.k_scan + V34_PARENTS_PER_TILE - 1) / V34_PARENTS_PER_TILE;
    ...
    for (int64_t task = 0; task < total_tiles_kv; ++task) {
        ...
        const int64_t parent_lo = tile * V34_PARENTS_PER_TILE;
        const int64_t parent_hi = std::min(parent_lo + V34_PARENTS_PER_TILE, p.k_scan);

        for (int64_t parent = parent_lo; parent < parent_hi; ++parent) {
            // Check if any group member has any surviving child in this block
            const uint32_t p_any = parent_any[kvh * p.k_total + parent];
            if (p_any == 0) continue;
            ...
            for (int64_t child = 0; child < p.bf; ++child) {
                ...
                const uint32_t c_mask = child_pass[kvh * p.n_pad + j];
                if (c_mask == 0) continue;

                // Read key/value once, score for all passing group members
                const float* key = krp + (kvh * p.n_pad + j) * p.d;
                const float* val = vrp + (kvh * p.n_pad + j) * p.d_v;
                for (int64_t g = 0; g < G; ++g) {
                    if (!((c_mask >> g) & 1u)) continue;
                    const int64_t qh = qh_list[static_cast<size_t>(g)];
