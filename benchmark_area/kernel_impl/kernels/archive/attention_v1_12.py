"""attention_v1.12 — v1.10 + num_stages=2 async pipelining for main attn kernel."""
...
from ._attention_triton_v1_12 import run_fused_attn_index_parentlist_pipelined

KERNEL_VERSION = "v1.12"
_PARENTS_PER_PROG = 8
...
def _launch_no_buffer(
    ...
    run_fused_attn_index_parentlist_pipelined(
        q=work["static_q"],
...
def _maybe_capture_graph(
    ...
    if work["graph"] is not None or work["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_12_use_cuda_graphs", True)):
...
def _get_fixed_empty_runtime(
    ...
    cache = state.setdefault("_attn_v1_12_fixed", {})
...
def attend(
    ...
    if not _empty_buffer(buffer_keys, buffer_values):
        raise RuntimeError("attention_v1_12 is a fixed-shape empty-buffer variant; use v1.10 for non-empty buffers")
