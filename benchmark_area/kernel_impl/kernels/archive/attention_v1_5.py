"""attention_v1.5 — v1.2 (workspace reuse) + fused q-pack into cluster_pass.

Eliminates the 4-op `_pack_q` Python path (view/transpose/contiguous + norm).
The cluster_pass kernel now loads each subspace's q slice directly from the
raw (H_q, D) tensor and computes the per-subspace ℓ2 norm on the fly.
"""
...
def _get_layout_attn(state: dict, q_head_to_kv: torch.Tensor | None, q: torch.Tensor):
    ...
    cache = state.setdefault("_attn_v1_5_layout", {})
    cache_key = (
        mode,
        groups,
        mapping_sig,
...
def attend(
    ...
    # Raw-q cluster_pass: no _pack_q. Reuse workspace buffer across calls.
    ws = state.setdefault("_attn_v1_5_ws", {})
