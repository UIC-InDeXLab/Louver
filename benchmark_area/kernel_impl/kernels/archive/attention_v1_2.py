"""attention_v1.2 — v1.0 with a persistent workspace + fewer .contiguous().

Differences from v1.0:
  - m_idx/l_idx/o_idx/out tensors live in the state as a cached workspace
    keyed on (h_q, num_splits, d_v); re-used across calls.
  - q is passed directly (assumed already contiguous from the caller path).
"""
...
def attend(
    ...
    # Persistent workspace for partials + output.
    ws = state.setdefault("_attn_v1_2_ws", {})
