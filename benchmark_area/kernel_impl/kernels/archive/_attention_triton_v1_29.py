"""v1.29 attention index kernel — dead-column masking plus group tiling.

This variant tiles grouped-query heads so a surviving head in one tile does not
keep the same parent-child column live for all query heads of the kv group.
"""
...
if HAS_TRITON:
    ...
    @triton.jit
    def _fused_attn_index_ns_group_tiled_kernel(
        Q_ptr,
        KeysBlocksT_ptr,
        ValuesBlocks_ptr,
        AssignsBlocks_ptr,
        ...
        m = tl.full([GROUPS_TILE], -1.0e30, dtype=tl.float32)
        l_acc = tl.zeros([GROUPS_TILE], dtype=tl.float32)
        o_acc = tl.zeros([GROUPS_TILE, D_V], dtype=tl.float32)
...
def run_fused_attn_index_ns_group_tiled(
    q: torch.Tensor,
    ...
    group_tiles = triton.cdiv(groups, groups_tile)
    grid = (h_kv_eff, num_splits, group_tiles)
    _fused_attn_index_ns_group_tiled_kernel[grid](
        ...
        GROUPS=groups, GROUPS_TILE=groups_tile,
