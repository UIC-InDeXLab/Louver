"""update_v2.1 — incremental update for build_v2.4 state.

Strategy: cluster the buffer alone with build_v2_4, then merge with the
existing state along the K axis (append K_buf new parents to the index).
No re-clustering of old parents, so cost scales with the buffer size,
not the total index size.

Output state is layout-compatible with build_v2_4 and usable by the
fastest attention kernels (v1.15, v1.15_s16, v1.5).
"""
...
def update(
    ...
    # Build a small index on the buffer alone.
    sub = build_v2_4(
        keys=buffer_keys,
        bf=bf,
    ...
    # Concatenate physical-layout tensors along the child/N_pad axis.
    ...
    # reorder_perm: offset buffer indices by N_old so it points into merged raw keys.
    reorder_perm_new = torch.cat(
        [state["reorder_perm"], sub["reorder_perm"] + n_old], dim=1
    ).contiguous()

    # assigns_reord (non-anchor + anchor): offset buffer cluster ids by K_old,
    # then concat along the N_pad axis.
    assigns_reord_new: list[torch.Tensor] = []
    for s_old, s_sub in zip(state["assigns_reord"], sub["assigns_reord"]):
        # s_sub is int32 (see build_v2_0); add K_old without overflow.
        merged = torch.cat([s_old, s_sub + k_old], dim=1).contiguous()
        assigns_reord_new.append(merged)

    # Per-subspace centers / radii: concat along K axis.
    ...
    # Block-packed tensors (keys_blocks_t, assigns_blocks, invalid_blocks_i8):
    # concat along the K axis.
    ...
    sub_blocks = sub["assigns_blocks"].to(assigns_dtype) + k_old  # offset non-anchor cluster ids
    assigns_blocks_new = torch.cat([old_blocks, sub_blocks], dim=2).contiguous()

    # Values: concat if present on both sides.
    new_state: dict = {
