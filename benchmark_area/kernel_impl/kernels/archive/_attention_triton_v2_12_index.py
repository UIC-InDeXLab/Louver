"""v2.12 fused index+buffer kernel with pre-computed child_survive mask.

Replaces the inline anchor gate + per-chunk AND loop (v2.6/v2.11) with
a single contiguous load from the pre-computed child_survive mask.
Uses exp2 throughout.
"""
...
if HAS_TRITON:
    ...
    @triton.jit
    def _fused_attn_index_buf_v2_12_kernel(
        Q_ptr,
        KeysBlocksT_ptr,
        ValuesBlocks_ptr,
        ChildSurvive_ptr,
        ...
        # ── Phase 1: indexed parents with pre-computed child_survive ──
...
def run_fused_attn_index_buf_v2_12(
    q: torch.Tensor,
    ...
    _fused_attn_index_buf_v2_12_kernel[grid](
        q,
        keys_blocks_t_f16,
        values_blocks_f16,
        child_survive,
