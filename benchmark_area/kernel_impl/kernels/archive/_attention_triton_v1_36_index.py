"""v1.36 attention index kernel — anchor-only gate with parent-level loads."""
...
if HAS_TRITON:
    ...
    @triton.jit
    def _fused_attn_index_anchor_fp16q_v1_36_kernel(
        Q_ptr,
        KeysBlocksT_ptr,
        ValuesBlocks_ptr,
        ...
        for p_chunk_start in range(p_start, p_end, PARENTS_PER_PROG):
            ...
            parent_idx_safe = tl.where(parent_valid, parent_idx, 0)

            col_valid = parent_valid[parent_rel]
            parent_idx_cols = parent_idx_safe[parent_rel]

            parent_mask = g_valid[:, None] & parent_valid[None, :]
            anchor_parent = tl.load(
                ClusterPassAnchor_ptr + hq_vec[:, None] * K + parent_idx_safe[None, :],
                mask=parent_mask,
                other=0,
            )
            survive = (anchor_parent[:, parent_rel] != 0) & (g_valid[:, None] & col_valid[None, :])

            inv = tl.load(
                InvalidBlocks_ptr + ((kvh * K + parent_idx_cols) * BF + child_rel),
                mask=col_valid,
                other=1,
            )
            ...
            if tl.max(live_cols.to(tl.int32), axis=0) != 0:
                keys_tile = tl.load(
                    KeysBlocksT_ptr
                    + ((kvh * K + parent_idx_cols[None, :]) * D + d_range[:, None]) * BF
                    + child_rel[None, :],
                    mask=live_cols[None, :],
                    other=0.0,
                )
                ...
                v_tile = tl.load(
                    ValuesBlocks_ptr
                    + ((kvh * K + parent_idx_cols[:, None]) * BF + child_rel[:, None]) * D_V
                    + dv_range[None, :],
                    mask=live_cols[:, None],
                    other=0.0,
                )
...
def run_fused_attn_index_anchor_fp16q_parentgate(
    q: torch.Tensor,
    ...
    _fused_attn_index_anchor_fp16q_v1_36_kernel[grid](
        q,
        keys_blocks_t_f16,
        values_blocks_f16,
        cluster_pass_anchor,
