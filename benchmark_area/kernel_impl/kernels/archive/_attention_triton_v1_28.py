"""v1.28 attention index kernel — skipdead plus masked metadata loads.

This variant keeps v1.20's all-dead chunk skip and additionally stops loading
per-subspace assign metadata for columns that are already dead for every query
head in the current program.
"""
...
if HAS_TRITON:
    ...
    @triton.jit
    def _fused_attn_index_ns_maskdead_kernel(
        Q_ptr,
        KeysBlocksT_ptr,
        ValuesBlocks_ptr,
        AssignsBlocks_ptr,
...
def run_fused_attn_index_ns_maskdead(
    q: torch.Tensor,
    ...
    _fused_attn_index_ns_maskdead_kernel[grid](
