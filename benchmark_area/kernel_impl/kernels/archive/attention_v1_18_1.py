"""attention_v1.18.1 — v1.18 with fp16 query/threshold/buffer inputs.

This variant is intended for apples-to-apples fp16 benchmarking:
  - q/buffer inputs must already be fp16.
  - `th_per_subspace` is a packed fp16 tensor with shape (2*S, H_q):
    thresholds in the first S rows and precomputed per-subspace q norms in the
    next S rows.
  - centers/radii are converted to fp16 once when the fixed runtime is built.
  - the timed path avoids fp32<->fp16 input conversions.

Online-softmax accumulators remain fp32, matching the existing sparse kernels.
"""
...
from ._attention_triton_v1_18_1_cluster import triton_fused_cluster_pass_rawq_g4_fp16
from ._attention_triton_v1_18_1 import run_fused_attn_index_fp16q_ns
from ._attention_buffer_triton_v1_18_1 import run_buffer_attn_fp16q
...
def _get_layout_fp16(
    ...
    layout = get_layout_attn_rawq(
        ...
        q_head_to_kv,
        q,
        cache_name="_attn_v1_18_1_layout",
    )
    cache = state.setdefault("_attn_v1_18_1_layout_fp16", {})
    cache_key = (
        layout["mode"],
        layout["groups"],
        layout["base_heads"],
...
def _launch_no_buffer(
    ...
    triton_fused_cluster_pass_rawq_g4_fp16(
        q=shared["static_q"],
...
def _capture_no_buffer_graph(
    ...
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_18_1_use_cuda_graphs", True)):
...
def _get_fixed_runtime(
    ...
    cache = state.setdefault("_attn_v1_18_1_fixed", {})
    ...
    if groups > _GROUPS_MAX:
        raise RuntimeError(
            f"attention_v1_18_1 requires groups <= {_GROUPS_MAX}; got {groups}"
        )
    ...
    if s_subspaces not in _ALLOWED_S:
        raise RuntimeError(
            f"attention_v1_18_1 requires S in {_ALLOWED_S}; got S={s_subspaces}"
        )
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v1_18_1 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v1_18_1 requires build_v2-style state")
    ...
    if th_per_subspace.dim() != 2 or th_per_subspace.shape[1] != h_q:
        raise RuntimeError(
            "attention_v1_18_1 expects packed fp16 thresholds with shape (2*S, H_q)"
        )
    rows = int(th_per_subspace.shape[0])
    s_hint = rows // 2
    if rows % 2 != 0 or s_hint not in _ALLOWED_S:
        raise RuntimeError(
            f"attention_v1_18_1 expects packed fp16 thresholds with 2*S rows "
    ...
    cfg.update((state.get("_attn_v1_18_1_buffer_cfg") or {}).get(bucket, {}))
