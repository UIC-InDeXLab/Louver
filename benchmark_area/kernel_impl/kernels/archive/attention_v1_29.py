"""attention_v1.29 — v1.28 plus grouped-head tiling in sparse index."""
...
def _require_supported(layout: dict) -> None:
    ...
    if s not in _ALLOWED_S:
        raise RuntimeError(
            f"attention_v1_29 requires S in {_ALLOWED_S}; got S={s}"
        )
    if int(layout["groups"]) > _GROUPS_MAX:
        raise RuntimeError(
            f"attention_v1_29 requires groups <= {_GROUPS_MAX}; got {layout['groups']}"
        )
...
def _capture_no_buffer_graph(state, shared, stage, layout, h_q, k, groups, groups_pow, num_splits, anchor_s, scale, parents_per_prog, s_subspaces):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_29_use_cuda_graphs", True)):
...
def _get_fixed_runtime(state, q, th, q_head_to_kv, num_splits):
    ...
    cache = state.setdefault("_attn_v1_29_fixed", {})
    fixed = cache.get("fixed")
    if cache.get("key") == cache_key and fixed is not None:
        return fixed
    layout = get_layout_attn_rawq(state, q_head_to_kv, q, cache_name="_attn_v1_29_layout")
...
def attend(q, th_per_subspace, state, buffer_keys, buffer_values, keys_children, q_head_to_kv=None, scale=None, num_splits=_DEFAULT_NUM_SPLITS):
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v1_29 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v1_29 requires build_v2-style state")
    h_q = q.shape[0]
    d = q.shape[1]
    q_c = q if q.is_contiguous() else q.contiguous()
    fixed_probe = state.get("_attn_v1_29_fixed", {}).get("fixed")
    ...
    if s_hint is None:
        ...
        if s_hint not in _ALLOWED_S:
            raise RuntimeError(f"attention_v1_29 requires S in {_ALLOWED_S}; inferred S={s_hint}")
    ...
    cfg.update((state.get("_attn_v1_29_buffer_cfg") or {}).get(bucket, {}))
