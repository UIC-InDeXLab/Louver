"""v2.11 fused index+buffer kernel with multi-subspace AND filter + exp2.

Based on v2.6 (fused buffer into index), but after the inline anchor gate
prunes parents, each surviving child is further AND-filtered against all
non-anchor subspaces via the pre-computed cluster_pass mask.
"""
...
if HAS_TRITON:
    ...
    @triton.jit
    def _fused_attn_index_buf_and_v2_11_kernel(
        Q_ptr,
        KeysBlocksT_ptr,
        ValuesBlocks_ptr,
        CentersAnchor_ptr,
        ...
        for p_chunk_start in range(p_start, p_end, PARENTS_PER_PROG):
            ...
            # AND with non-anchor subspaces via pre-computed cluster_pass
...
def run_fused_attn_index_buf_and_v2_11(
    q: torch.Tensor,
    ...
    _fused_attn_index_buf_and_v2_11_kernel[grid](
        q,
        keys_blocks_t_f16,
        values_blocks_f16,
        centers_anchor,
