"""attention_v1.24 — v1.20 with on-chip reduction across split groups."""
...
from ._attention_triton_v1_24 import run_fused_attn_index_grouped_splits
...
def _outer_splits(num_splits: int) -> int:
    return (num_splits + _SPLITS_PER_PROG - 1) // _SPLITS_PER_PROG
...
def _require_supported(layout: dict) -> None:
    ...
    if s not in _ALLOWED_S:
        raise RuntimeError(
            f"attention_v1_24 requires S in {_ALLOWED_S}; got S={s}"
        )
    if int(layout["groups"]) > _GROUPS_MAX:
        raise RuntimeError(
            f"attention_v1_24 requires groups <= {_GROUPS_MAX}; got {layout['groups']}"
        )
...
def _make_shared_workspace(layout: dict, q: torch.Tensor, th: torch.Tensor, num_splits: int) -> dict:
    ...
    k = layout["K"]
    outer_splits = _outer_splits(num_splits)
    device = q.device
    return {
        ...
        "m_idx": torch.empty(h_q, outer_splits, device=device, dtype=torch.float32),
        "l_idx": torch.empty(h_q, outer_splits, device=device, dtype=torch.float32),
        "o_idx": torch.empty(h_q, outer_splits, d_v, device=device, dtype=torch.float32),
...
def _launch_no_buffer(
    ...
    run_fused_attn_index_grouped_splits(
        q=shared["static_q"],
        ...
        splits_per_prog=_SPLITS_PER_PROG,
        anchor_s=anchor_s,
        scale=scale,
        out_m=shared["m_idx"],
        out_l=shared["l_idx"],
...
def _capture_no_buffer_graph(
    ...
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_24_use_cuda_graphs", True)):
...
def _get_fixed_runtime(
    ...
    cache = state.setdefault("_attn_v1_24_fixed", {})
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v1_24 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v1_24 requires build_v2-style state")

    h_q = q.shape[0]
    d = q.shape[1]
    q_c = q if q.is_contiguous() else q.contiguous()

    fixed_probe = state.get("_attn_v1_24_fixed", {}).get("fixed")
    ...
    if s_hint is None:
        ...
        if s_hint not in _ALLOWED_S:
            raise RuntimeError(
                f"attention_v1_24 requires S in {_ALLOWED_S}; inferred S={s_hint}"
            )
    ...
    cfg.update((state.get("_attn_v1_24_buffer_cfg") or {}).get(bucket, {}))
