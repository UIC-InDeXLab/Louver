"""attention_v1.1 — v1.0 with a tighter GROUPS_POW floor.

Only difference vs v1.0: `groups_pow = max(next_pow2(groups), 4)` instead
of `max(... , 8)`. For groups=3 (H_q=24/H_kv=8) the tensor-core dot used
to produce 8 output rows with 5 wasted; now it produces 4 with 1 wasted.
"""
...
def attend(
    ...
    k = layout["K"]
    groups = layout["groups"]
    groups_pow = max(_next_pow2(groups), 4)  # v1.1: was 8
