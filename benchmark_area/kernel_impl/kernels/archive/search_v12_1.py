"""search_v12.1 — autotuned parent-major variant with dense output writes.

This keeps the promising parts of v12.0:
  - Triton autotune over launch shape
  - parent-major assign layout

but uses the dense write path from v11 instead of sparse writes with output
prefill, which regressed in benchmarks.
"""
...
def _get_or_make_parent_major_pack(
    ...
    cache = state.setdefault("_search_v12_1_parent_major_pack", {})
    cache_key = (
        keys_reord.data_ptr(),
...
if HAS_TRITON:
    ...
    @triton.autotune(configs=_AUTOTUNE_CONFIGS, key=["K", "H_Q"])
    @triton.jit
    def _fused_anchor_parent_batch_dense_kernel(
        Q_ptr,                 # (H_q, D)
...
def _get_layout_v12_1(state: dict, q_head_to_kv: torch.Tensor | None, q: torch.Tensor):
    ...
    cache = state.setdefault("_search_v12_1_cache", {})
    cache_key = (
        mode,
        groups,
        mapping_sig,
...
def search(
    ...
    if not HAS_TRITON:
        raise RuntimeError("search_v12_1 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("search_v12_1 requires build_v2-style state")

    layout = _get_layout_v12_1(state, q_head_to_kv, q)
    h_q = q.shape[0]
