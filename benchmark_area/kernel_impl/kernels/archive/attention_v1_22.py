"""attention_v1.22 — v1.20 without the materialized cluster_pass tensor.

This variant keeps v1.20's best ideas:
  - leaner grouped-query lane shape
  - incremental append-only buffer staging
  - skip of all-dead sparse chunks

But it removes the separate cluster_pass launch and buffer. The sparse
attention kernel computes the subspace upper-bound gates on demand.
"""
...
from ._attention_triton_v1_22 import run_ondemand_fused_attn_index_ns
...
def _require_supported(layout: dict) -> None:
    ...
    if s not in _ALLOWED_S:
        raise RuntimeError(
            f"attention_v1_22 requires S in {_ALLOWED_S}; got S={s}"
        )
    if int(layout["groups"]) > _GROUPS_MAX:
        raise RuntimeError(
            f"attention_v1_22 requires groups <= {_GROUPS_MAX}; got {layout['groups']}"
        )
...
def _make_shared_workspace(layout: dict, q: torch.Tensor, th: torch.Tensor, num_splits: int) -> dict:
    ...
    return {
        ...
        "q_packed": torch.empty(s, h_q, max_d, device=device, dtype=torch.float32),
        "q_norm": torch.empty(s, h_q, device=device, dtype=torch.float32),
...
def _prepare_q_inputs(shared: dict, layout: dict) -> None:
    q = shared["static_q"]
    q_packed = shared["q_packed"]
    h_q = q.shape[0]
    s = layout["num_subspaces"]
    max_d = layout["max_d"]
    d = q.shape[1]
    ...
    if d == s * max_d:
        q_packed.copy_(q.view(h_q, s, max_d).transpose(0, 1))
    else:
        q_packed.zero_()
        for si, (s0, e0) in enumerate(layout["dim_slices"]):
            q_packed[si, :, : e0 - s0].copy_(q[:, s0:e0])
    torch.linalg.vector_norm(q_packed, dim=-1, out=shared["q_norm"])
...
def _launch_no_buffer(
    ...
    _prepare_q_inputs(shared, layout)

    run_ondemand_fused_attn_index_ns(
        q=shared["static_q"],
        q_packed=shared["q_packed"],
        q_norm=shared["q_norm"],
        th=shared["static_th"],
...
def _capture_no_buffer_graph(
    ...
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_22_use_cuda_graphs", True)):
...
def _get_fixed_runtime(
    ...
    cache = state.setdefault("_attn_v1_22_fixed", {})
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v1_22 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v1_22 requires build_v2-style state")

    h_q = q.shape[0]
    d = q.shape[1]
    q_c = q if q.is_contiguous() else q.contiguous()

    fixed_probe = state.get("_attn_v1_22_fixed", {}).get("fixed")
    ...
    if s_hint is None:
        ...
        if s_hint not in _ALLOWED_S:
            raise RuntimeError(
                f"attention_v1_22 requires S in {_ALLOWED_S}; inferred S={s_hint}"
            )
    ...
    cfg.update((state.get("_attn_v1_22_buffer_cfg") or {}).get(bucket, {}))
