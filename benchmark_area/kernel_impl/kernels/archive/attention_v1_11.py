"""attention_v1.11 — v1.10 + dead-chunk early exit in the main attention kernel."""
...
from ._attention_triton_v1_11 import run_fused_attn_index_parentlist_skip
...
def _launch_no_buffer(
    ...
    run_fused_attn_index_parentlist_skip(
        q=work["static_q"],
...
def _maybe_capture_graph(
    ...
    if work["graph"] is not None or work["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_11_use_cuda_graphs", True)):
...
def _get_fixed_empty_runtime(
    ...
    cache = state.setdefault("_attn_v1_11_fixed", {})
...
def attend(
    ...
    if not _empty_buffer(buffer_keys, buffer_values):
        raise RuntimeError("attention_v1_11 is a fixed-shape empty-buffer variant; use v1.10 for non-empty buffers")
