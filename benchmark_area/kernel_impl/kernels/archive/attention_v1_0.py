"""attention_v1.0 — fused sparse-index attention.

Replaces the (search → cat → softmax → @V) pipeline with a single call that
returns standard LLM attention output directly:

    out = softmax(scale * q @ K^T) @ V    over (indexed + buffered) K/V

The index-side K/V are gated by cluster_pass (same pruning as search_v15) and
consumed by a Triton kernel that accumulates FlashAttention-style (m, l, o)
partials per split. The buffer-side K/V are processed in a small dense torch
path that produces one matching partial. A Triton reduce kernel merges all
partials into the final (H_q, D_v) output.

Conventions:
  - No causal mask in-kernel: every indexed / buffered pair is a valid past
    attention target; masking is the caller's responsibility via what it
    puts into the index and buffer.
  - `scale` defaults to 1/sqrt(D_head).
  - Requires build_v2.4 state (has `values_blocks_f16` and `keys_blocks_t_f16`).
"""
