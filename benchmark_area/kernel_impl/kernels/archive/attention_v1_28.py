"""attention_v1.28 — v1.18 with auto-anchor build and dead-column masking."""
...
from ._attention_triton_v1_28 import run_fused_attn_index_ns_maskdead
...
def _require_supported(layout: dict) -> None:
    ...
    if s not in _ALLOWED_S:
        raise RuntimeError(
            f"attention_v1_28 requires S in {_ALLOWED_S}; got S={s}"
        )
    if int(layout["groups"]) > _GROUPS_MAX:
        raise RuntimeError(
            f"attention_v1_28 requires groups <= {_GROUPS_MAX}; got {layout['groups']}"
        )
...
def _launch_no_buffer(
    ...
    run_fused_attn_index_ns_maskdead(
        q=shared["static_q"],
...
def _capture_no_buffer_graph(
    ...
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_28_use_cuda_graphs", True)):
...
def _get_fixed_runtime(
    ...
    cache = state.setdefault("_attn_v1_28_fixed", {})
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v1_28 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v1_28 requires build_v2-style state")

    h_q = q.shape[0]
    d = q.shape[1]
    q_c = q if q.is_contiguous() else q.contiguous()

    fixed_probe = state.get("_attn_v1_28_fixed", {}).get("fixed")
    ...
    if s_hint is None:
        ...
        if s_hint not in _ALLOWED_S:
            raise RuntimeError(
                f"attention_v1_28 requires S in {_ALLOWED_S}; inferred S={s_hint}"
            )
    ...
    cfg.update((state.get("_attn_v1_28_buffer_cfg") or {}).get(bucket, {}))
