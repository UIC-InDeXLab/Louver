"""attention_v2.4 — v2.0 (exp2) with 8 warps for index kernel + num_splits=85.

More warps may improve occupancy on the memory-bound index kernel at the
cost of fewer registers per thread.
"""
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v2_4_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v2_4 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v2_4 requires build_v2-style state")
    ...
    cache = state.setdefault("_attn_v2_4_fixed", {})
    ...
    cfg.update((state.get("_attn_v2_4_buffer_cfg") or {}).get(bucket, {}))
