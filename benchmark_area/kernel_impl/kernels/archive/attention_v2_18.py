"""attention_v2.18 — v2.15 with optimized auxiliary kernels.

Changes vs v2.15:
  - cluster_pass: BLOCK_K=32 (more thread blocks, better SM utilization)
  - child_survive: BLOCK_C=1024, num_warps=2 (fewer blocks, more work per thread)
  - num_splits=85 (same as v2.15)
"""
...
from ._attention_triton_v2_18_aux import run_cluster_pass_fast, run_child_survive_fast
...
def _launch_no_buffer(shared, layout, h_q, k, groups, groups_pow,
    ...
    run_cluster_pass_fast(
        ...
        block_k=_CLUSTER_BLOCK_K,
    )
    run_child_survive_fast(
        ...
        block_c=_SURVIVE_BLOCK_C, num_warps=_SURVIVE_WARPS,
    )
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v2_18_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v2_18 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v2_18 requires build_v2-style state")
    ...
    cache = state.setdefault("_attn_v2_18_fixed", {})
