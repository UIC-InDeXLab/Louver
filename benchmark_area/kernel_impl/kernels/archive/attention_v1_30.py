"""attention_v1.30 — v1.29 plus fused buffer scan and final reduce."""
...
from ._attention_triton_v1_29 import run_fused_attn_index_ns_group_tiled
from ._attention_reduce_buffer_triton_v1_30 import run_attn_reduce_buffer
...
def _require_supported(layout: dict) -> None:
    ...
    if s not in _ALLOWED_S:
        raise RuntimeError(
            f"attention_v1_30 requires S in {_ALLOWED_S}; got S={s}"
        )
    if int(layout["groups"]) > _GROUPS_MAX:
        raise RuntimeError(
            f"attention_v1_30 requires groups <= {_GROUPS_MAX}; got {layout['groups']}"
        )
...
def _launch_no_buffer(shared, layout, h_q, k, groups, groups_pow, num_splits, anchor_s, scale, parents_per_prog, s_subspaces):
    del groups_pow
    ...
    run_fused_attn_index_ns_group_tiled(
        q=shared["static_q"],
...
def _launch_with_buffer(shared, stage, layout, h_q, k, groups, groups_pow, num_splits, anchor_s, scale, parents_per_prog, s_subspaces, bucket, buf_cols, buf_warps, buf_stages):
    del groups_pow, buf_warps, buf_stages
    triton_fused_cluster_pass_rawq_g4(
        q=shared["static_q"],
        th=shared["static_th"],
        dim_offsets=layout["dim_offsets"],
...
def _fixed_cache_key(state, q, th, q_head_to_kv, num_splits):
...
def _capture_no_buffer_graph(state, shared, stage, layout, h_q, k, groups, groups_pow, num_splits, anchor_s, scale, parents_per_prog, s_subspaces):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_30_use_cuda_graphs", True)):
    ...
    try:
        with torch.cuda.stream(stream):
            for _ in range(3):
                _launch_no_buffer(shared, layout, h_q, k, groups, groups_pow, num_splits, anchor_s, scale, parents_per_prog, s_subspaces)
...
def _capture_with_buffer_graph(state, shared, stage, layout, h_q, k, groups, groups_pow, num_splits, anchor_s, scale, parents_per_prog, s_subspaces, bucket, buf_cols, buf_warps, buf_stages):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v1_30_use_cuda_graphs", True)):
        stage["capture_failed"] = True
    ...
    try:
        with torch.cuda.stream(stream):
            for _ in range(3):
                _launch_with_buffer(shared, stage, layout, h_q, k, groups, groups_pow, num_splits, anchor_s, scale, parents_per_prog, s_subspaces, bucket, buf_cols, buf_warps, buf_stages)
        current.wait_stream(stream)
        graph = torch.cuda.CUDAGraph()
        with torch.cuda.graph(graph):
            _launch_with_buffer(shared, stage, layout, h_q, k, groups, groups_pow, num_splits, anchor_s, scale, parents_per_prog, s_subspaces, bucket, buf_cols, buf_warps, buf_stages)
...
def _get_fixed_runtime(state, q, th, q_head_to_kv, num_splits):
    cache_key = _fixed_cache_key(state, q, th, q_head_to_kv, num_splits)
    cache = state.setdefault("_attn_v1_30_fixed", {})
    fixed = cache.get("fixed")
    if cache.get("key") == cache_key and fixed is not None:
        return fixed
    layout = get_layout_attn_rawq(state, q_head_to_kv, q, cache_name="_attn_v1_30_layout")
...
def attend(q, th_per_subspace, state, buffer_keys, buffer_values, keys_children, q_head_to_kv=None, scale=None, num_splits=_DEFAULT_NUM_SPLITS):
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v1_30 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v1_30 requires build_v2-style state")
    h_q = q.shape[0]
    d = q.shape[1]
    q_c = q if q.is_contiguous() else q.contiguous()
    fixed_probe = state.get("_attn_v1_30_fixed", {}).get("fixed")
    ...
    if s_hint is None:
        ...
        if s_hint not in _ALLOWED_S:
            raise RuntimeError(f"attention_v1_30 requires S in {_ALLOWED_S}; inferred S={s_hint}")
    ...
    if empty:
        ...
        _capture_no_buffer_graph(state, shared, stage, layout, h_q, k, groups, groups_pow, num_splits, anchor_s, scale, parents_per_prog, s_subspaces)
    ...
    buf_keys_eff, buf_values_eff = _prepare_buffer_effective(buffer_keys, buffer_values, layout, q_head_to_kv)
    _copy_buffer_into_stage_incremental(stage, buf_keys_eff, buf_values_eff)
    cfg = dict(_DEFAULT_BUFFER_CFG[bucket])
    cfg.update((state.get("_attn_v1_30_buffer_cfg") or {}).get(bucket, {}))
    _capture_with_buffer_graph(state, shared, stage, layout, h_q, k, groups, groups_pow, num_splits, anchor_s, scale, parents_per_prog, s_subspaces, bucket, cfg["cols"], cfg["num_warps"], cfg["num_stages"])
    if stage["graph"] is not None:
        stage["graph"].replay()
    else:
        _launch_with_buffer(shared, stage, layout, h_q, k, groups, groups_pow, num_splits, anchor_s, scale, parents_per_prog, s_subspaces, bucket, cfg["cols"], cfg["num_warps"], cfg["num_stages"])
    return shared["out"]
