"""attention_v2.14 — hybrid: v2.6 anchor gate + guarded child_survive AND.

Pipeline (4 launches, all captured in a single CUDA graph):
  1. cluster_pass:  per-subspace parent gating (reused from v1.31)
  2. child_survive: fuses cluster_pass + assigns_blocks + invalid into a
     flat (H_q, K*BF) int8 mask — one Triton kernel
  3. fused_index+buffer: inline anchor gate prunes ~80-90% of chunks,
     child_survive loaded ONLY for surviving chunks
  4. reduce: merge split partials (exp2)
"""
...
from ._attention_triton_v2_14_index import run_fused_attn_index_buf_v2_14
...
def _make_shared_v2_14(layout, q, th_view, num_splits):
    base = _make_shared_workspace(layout, q, th_view, num_splits)
    h_q = q.shape[0]
    s = layout["num_subspaces"]
    k = layout["K"]
    bf = layout["bf"]
    base["cluster_pass"] = torch.empty(
        s, h_q, k, device=q.device, dtype=torch.int8,
    )
    base["child_survive"] = torch.empty(
        h_q, k * bf, device=q.device, dtype=torch.int8,
    )
    return base
...
def _launch_with_buffer(shared, stage, layout, h_q, k, groups, groups_pow,
    ...
    run_fused_attn_index_buf_v2_14(
        q=shared["static_q"],
        keys_blocks_t_f16=layout["keys_blocks_t_f16"],
        values_blocks_f16=layout["values_blocks_f16"],
        centers_anchor=anchor["centers"], radii_anchor=anchor["radii"],
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v2_14_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v2_14 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v2_14 requires build_v2-style state")
    ...
    cache = state.setdefault("_attn_v2_14_fixed", {})
    ...
    if cache.get("key") != cache_key or fixed is None:
        ...
        shared = _make_shared_v2_14(layout, q_c, th_view, num_splits)
        fixed = {
