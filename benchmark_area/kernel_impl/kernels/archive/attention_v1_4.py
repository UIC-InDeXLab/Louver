"""attention_v1.4 — v1.2 (workspace reuse) + autotuned launch params.

Replaces the hardcoded (PARENTS_PER_PROG=8, num_warps=4, num_stages=1)
launch with a `@triton.autotune` variant that searches over
PARENTS_PER_PROG ∈ {4, 8, 16}, num_warps ∈ {2, 4, 8}, num_stages ∈ {1, 2, 3}.
First call on a new (K, BF, D, D_V, S, GROUPS_POW, NUM_SPLITS) key pays the
autotune cost; warmed runs use the winner.
"""
...
from ._attention_triton_v1_4 import run_fused_attn_index_autotuned
...
def attend(
    ...
    ws = state.setdefault("_attn_v1_4_ws", {})
    ws_key = (h_q, num_splits, d_v, q.device.index)
    ...
    run_fused_attn_index_autotuned(
