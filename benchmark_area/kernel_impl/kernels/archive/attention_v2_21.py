"""attention_v2.21 — v2.15 with copy_q_th moved inside CUDA graph.

Pipeline (5 launches, ALL inside CUDA graph — no external launch):
  0. copy_q_th (inside graph)
  1. cluster_pass (tl.dot)
  2. child_survive
  3. fused_index+buffer
  4. reduce (exp2)
"""
...
def _make_shared_v2_21(layout, q, packed, num_splits):
    base = _make_shared_workspace(layout, q, packed[:packed.shape[0]//2], num_splits)
    h_q = q.shape[0]
    ...
    base["q_src"] = torch.empty_like(q)
    base["th_src"] = torch.empty((2 * s, h_q), device=q.device, dtype=torch.float16)
    return base
...
def _launch_no_buffer(shared, layout, h_q, k, groups, groups_pow,
                      ...
                      s_subspaces, anchor):
    fused_copy_q_th(shared["q_src"], shared["th_src"],
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v2_21_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v2_21 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v2_21 requires build_v2-style state")
    ...
    cache = state.setdefault("_attn_v2_21_fixed", {})
    ...
    if cache.get("key") != cache_key or fixed is None:
        ...
        shared = _make_shared_v2_21(layout, q_c, packed, num_splits)
        fixed = {
    ...
    shared["q_src"].copy_(q_c)
    shared["th_src"].copy_(packed)
