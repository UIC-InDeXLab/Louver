"""ta_filter v9.0 — Phases 1+2 of v7.10 plus a bitmap-dump Phase 3.

Outputs:
    parent_alive_bitmap: (Hq, 4, K_words) int32 — per-subspace bitmap of
        parents whose centroid score is in the top-(depth) of that
        subspace.  A key with index n is alive in subspace s iff
        ``bitmap[hq, s, p_s >> 5] & (1 << (p_s & 31))`` where p_s is the
        subspace-s parent of key n (read from ``_assigns_packed_u64_v34``).
    top_scores, top_indices, depth: as in v7.10 (kept for diagnostics).

Sparse-attn v2.6 consumes the bitmap + assigns_packed directly — no
live_idx materialization.
"""
...
def _load_ext():
    ...
    _EXT = load(
        name="ta_filter_v9_0",
        sources=[
            str(base / "ta_filter_v_9_0.cpp"),
            str(base / "ta_filter_cuda_kernel_v_9_0.cu"),
        ],
        extra_cflags=["-O3"],
...
def ta_filter_v9_0(q, threshold, state, q_head_to_kv=None, *, ws=None):
    if q.dtype != torch.float16:
        raise TypeError("ta_filter_v9_0 expects q fp16")
    ...
    if int(centers.shape[0]) != 4 or int(state["bf"]) != 4:
        raise ValueError("ta_filter_v9_0 requires S=4, bf=4")
    ...
    elif ws["parent_alive_bitmap"].shape[2] < k_words:
        raise ValueError(
            f"caller workspace bitmap last dim must be >= K_words={k_words}"
        )
