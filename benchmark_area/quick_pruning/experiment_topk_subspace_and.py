#!/usr/bin/env python3
"""
Experiment: derive per-subspace top-k thresholds and test an AND gate.
...
- filter all keys with an AND gate: keep a key only if it passes every
  subspace threshold q_i · x_i >= t_i
...
- |H \\ O|: full-space halfspace points missed by the AND gate
"""
...
def main() -> None:
    ...
    global_with_outside = 0
    global_extra_total = 0
    global_extra_inside = 0
    global_extra_outside = 0
    global_halfspace_total = 0
    global_missed_halfspace = 0
    ...
    for layer in selected_layers:
        ...
        layer_with_outside = 0
        layer_extra_total = 0
        layer_extra_inside = 0
        layer_extra_outside = 0
        layer_halfspace_total = 0
        layer_missed_halfspace = 0
        ...
        for qi in q_indices:
            ...
            keep_mask = torch.stack(subspace_passes, dim=0).all(dim=0)

            extra_mask = keep_mask & ~topk_mask
            extra_inside = extra_mask & halfspace_mask
            extra_outside = extra_mask & ~halfspace_mask
            missed_halfspace_mask = halfspace_mask & ~keep_mask

            extra_total_per_head = extra_mask.sum(dim=1)
            extra_inside_per_head = extra_inside.sum(dim=1)
            extra_outside_per_head = extra_outside.sum(dim=1)
            halfspace_total_per_head = halfspace_mask.sum(dim=1)
            missed_halfspace_per_head = missed_halfspace_mask.sum(dim=1)

            for head in range(h_eval):
                et = int(extra_total_per_head[head].item())
                ei = int(extra_inside_per_head[head].item())
                eo = int(extra_outside_per_head[head].item())
                ht = int(halfspace_total_per_head[head].item())
                mh = int(missed_halfspace_per_head[head].item())

                layer_cases += 1
                layer_extra_total += et
                layer_extra_inside += ei
                layer_extra_outside += eo
                layer_halfspace_total += ht
                layer_missed_halfspace += mh

                if et > 0:
                    layer_with_any_extra += 1
                if eo > 0:
                    layer_with_outside += 1
        ...
        global_with_outside += layer_with_outside
        global_extra_total += layer_extra_total
        global_extra_inside += layer_extra_inside
        global_extra_outside += layer_extra_outside
        global_halfspace_total += layer_halfspace_total
        global_missed_halfspace += layer_missed_halfspace

        print(f"  cases:                       {layer_cases}")
        print(f"  cases with |O \\\\ T| > 0:      {layer_with_any_extra} ({layer_with_any_extra / max(1, layer_cases):.2%})")
        print(f"  cases with |O \\\\ H| > 0:      {layer_with_outside} ({layer_with_outside / max(1, layer_cases):.2%})")
        print(f"  avg |O \\\\ T| / case:          {layer_extra_total / max(1, layer_cases):.3f}")
        print(f"  avg |(O ∩ H) \\\\ T| / case:    {layer_extra_inside / max(1, layer_cases):.3f}")
        print(f"  avg |O \\\\ H| / case:          {layer_extra_outside / max(1, layer_cases):.3f}")
        print(f"  avg |H \\\\ O| / case:          {layer_missed_halfspace / max(1, layer_cases):.3f}")
        print(f"  total |O \\\\ T|:               {layer_extra_total}")
        print(f"  total |(O ∩ H) \\\\ T|:         {layer_extra_inside}")
        print(f"  total |O \\\\ H|:               {layer_extra_outside}")
        print(f"  total |H|:                     {layer_halfspace_total}")
        print(f"  total |H \\\\ O|:               {layer_missed_halfspace}")

    print("\n" + "=" * 90)
    print("Global Summary")
    print(f"cases:                         {global_cases}")
    print(f"cases with |O \\\\ T| > 0:        {global_with_any_extra} ({global_with_any_extra / max(1, global_cases):.2%})")
    print(f"cases with |O \\\\ H| > 0:        {global_with_outside} ({global_with_outside / max(1, global_cases):.2%})")
    print(f"avg |O \\\\ T| / case:            {global_extra_total / max(1, global_cases):.3f}")
    print(f"avg |(O ∩ H) \\\\ T| / case:      {global_extra_inside / max(1, global_cases):.3f}")
    print(f"avg |O \\\\ H| / case:            {global_extra_outside / max(1, global_cases):.3f}")
    print(f"avg |H \\\\ O| / case:            {global_missed_halfspace / max(1, global_cases):.3f}")
    print(f"total |O \\\\ T|:                 {global_extra_total}")
    print(f"total |(O ∩ H) \\\\ T|:           {global_extra_inside}")
    print(f"total |O \\\\ H|:                 {global_extra_outside}")
    print(f"total |H|:                       {global_halfspace_total}")
    print(f"total |H \\\\ O|:                 {global_missed_halfspace}")

    if worst_cases:
        worst_cases.sort(
            key=lambda row: (
                int(row["missed_halfspace"]),
        ...
        for row in worst_cases[: args.max_report]:
            print(
                ...
                f"|O\\H|={row['extra_outside']} |H\\O|={row['missed_halfspace']} "
                f"|H|={row['halfspace_total']} "
