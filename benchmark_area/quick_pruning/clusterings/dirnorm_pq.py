"""PQ clustering on direction plus norm features."""
...
def cluster_dirnorm_pq(
    keys: torch.Tensor,
    bf: int,
    n_subspaces: int = 4,
    max_iter: int = 10,
    norm_weight: float = 4.0,
):
    """
    Cluster using L2-normalised directions augmented with a scaled norm
    feature, then rebuild centroids in the original key space.
    """
    H, N, D = keys.shape
    ...
    norm_feat = key_norms.log()
    norm_feat = (norm_feat - norm_feat.mean(dim=1, keepdim=True)) / norm_feat.std(dim=1, keepdim=True).clamp_min(1e-4)
    features = torch.cat([dir_keys, norm_weight * norm_feat], dim=-1)

    mean = features.mean(dim=1, keepdim=True)
    scale = features.std(dim=1, keepdim=True).clamp_min(1e-4)
    norm_features = (features - mean) / scale

    fd = norm_features.shape[-1]
    sub_dim = fd // n_subspaces
    remainder = fd % n_subspaces
    ...
    for s in range(n_subspaces):
        ...
        sub_keys = norm_features[:, :, offset : offset + sd].contiguous()
        offset += sd
    ...
    center_norms = centers.norm(dim=-1, keepdim=True).clamp_min(1e-12)
    center_dir = centers / center_norms
    center_norm_feat = center_norms.log()
    center_norm_feat = (center_norm_feat - norm_feat.mean(dim=1, keepdim=True)) / norm_feat.std(dim=1, keepdim=True).clamp_min(1e-4)
    center_features = torch.cat([center_dir, norm_weight * center_norm_feat], dim=-1)
    norm_center_features = (center_features - mean) / scale
    assign = torch.cdist(norm_features, norm_center_features).argmin(dim=2)
    centers = _centers_from_assign(keys, assign, K)
